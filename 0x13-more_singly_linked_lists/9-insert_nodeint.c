...
/**
 * insert_nodeint_at_index - This is a function that inserts a new node at
 *                           a given position
 *
...
listint_t *insert_nodeint_at_index(listint_t **head, unsigned int idx, int n)
{
	...
	current = *head; /*to place first node at current*/

	new_node = malloc(sizeof(listint_t));
	if ((*head == NULL && idx != 0) || new_node == NULL)
		return (NULL);

	new_node->n = n; /*to add our element to the new node*/

	/*to iterate list to node position idx - 2*/
